    ...
    if st.session_state.coddir!='':
        ...
        if datamarketventa.empty is False:
            ...
            for i, inmueble in datamarketventa.iterrows():
                ...
                url_export   = f"buydepa-app-colombia.streamlit.app/Ficha?code={inmueble['code']}&tiponegocio=Venta&tipoinmueble=Apartamento" 
        ...
        if datamarketarriendo.empty is False:
            ...
            for i, inmueble in datamarketarriendo.iterrows():
                ...
                propertyinfo = f'<strong>{inmueble["areaconstruida"]}</strong> mt<sup>2</sup> | <strong>{int(inmueble["habitaciones"])}</strong> hab | <strong>{int(inmueble["banos"])}</strong> baños {garajes_inmueble}'
                url_export   = f"buydepa-app-colombia.streamlit.app/Ficha?code={inmueble['code']}&tiponegocio=Arriendo&tipoinmueble=Apartamento" 
        ...
        with col3:
            if st.session_state.datacomparables.empty is False:
                ...
                for i, inmueble in st.session_state.datacomparables.iterrows():
                    ...
                    url_export   = f"buydepa-app-colombia.streamlit.app/Ficha?code={inmueble['code']}&tiponegocio={tiponegocio_comparables}&tipoinmueble={tipoinmueble_comparables}" 
                    string_popup = f'''
                    <!DOCTYPE html>
                    <html>
                      <head>
