    ...
    with col1:
        ...
        if st.session_state.polygonfilter is not None:
            ...
            for i, inmueble in st.session_state.data.iterrows():
                ...
                url_export   = f"http://https://buydepa-app-colombia.streamlit.app//Ficha?code={inmueble['code']}&tiponegocio={st.session_state.tiponegocio}&tipoinmueble={st.session_state.tipoinmueble}" 
                string_popup = f'''
                <!DOCTYPE html>
                <html>
                  <head>
